  ...
  */
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      alert("Congratulation. You are authenticated now!")
      router.push('/')
    } catch (err: any) {
  ...
  return (
    <div className="px-6 py-12 lg:px-8">
      <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        <p className='text-center font-bold text-xl mb-10'>Log in Vancooler Account</p>
        <form method="POST" className="flex flex-col gap-4">
          <div>
            <label htmlFor="email" className="font-medium text text-sm/6 font-medium text-gray-900">Email address</label>
          ...
          <div>
            <label htmlFor="password" className="font-medium text-sm/6 font-medium text-gray-900">Password</label>
          ...
          <button onClick={handleSubmit} type="submit" className="flex w-full justify-center rounded-md bg-indigo-600 px-3 py-1.5 text-sm/6 font-semibold text-white shadow-xs hover:bg-indigo-500">Sign in</button>
        </form>
      </div>
    </div>
  )
