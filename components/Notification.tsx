...
const sunscreen =
    `<
    dialog id="notification" className="rounded-xl p-5 fixed place-self-center max-w-90 max-h-9.9/10 flex-auto">
        ...
        <div className="flex">
            <button type="button" command="close" commandfor="notification" className="bg-gray-700 hover:bg-gray-800 text-white rounded-lg grow p-1">Exit</button>
        </div>
    </dialog >`
...
const uv = `
            <dialog id="notification" className="rounded-xl p-5 fixed place-self-center max-w-90 max-h-9.9/10 flex-auto">
                ...
                <h2 className="text-center text-xl">UV index right now is {String(uvIndex)}</h2>
...
export default function Notification(props: NotificationProps) {
    ...
    const openTime = timeOfNotif || null;
    const now = new Date();

    if (now.getDate() == openTime?.getDate()) {
        setOpen(true);
    }

    return (
        <dialog open={open} id="notification" className="rounded-xl p-5 fixed place-self-center max-w-90 max-h-9.9/10 flex-auto">
